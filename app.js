...
app.use(session({
  ...
  store: MongoStore.create({ mongoUrl: 'mongodb+srv://johnllentv:johnllentv@cluster0.pgaelxg.mongodb.net/Diaries?retryWrites=true&w=majority&appName=Cluster0' }),
  rolling: true,
  cookie: {
    maxAge: 15 * 60 * 1000,
...
app.post('/upload-csv', uploadCsv.fields([
...
const WORDPRESS_APPLICATION_PASSWORD = 'root'; 
  ...
  try {
    ...
    for (const consentForm of acceptedConsentForms) {
      ...
      if (student && student.picture) {
        ...
        fs.writeFileSync(filePath, student.picture);

        const formData = new FormData();
        formData.append('file', fs.createReadStream(filePath), `${studentNumber}.jpg`);
        formData.append('title', studentNumber);
        ...
        try {
          ...
          console.log(`Uploaded picture for student ${studentNumber}`);
...
const handleConsentForm = async (consentId) => {
  const consentForm = await ConsentForm.findById(consentId).populate('student_Number');

  if (consentForm.form_Status === "agree") {
    const yearbook = await Yearbook.findOne({ status: 'published' });

    if (new Date() < yearbook.consentDeadline) {
      const student = await Student.findOne({ studentNumber: consentForm.student_Number });
      
      const picture = student.picture;
      
      const query = 'INSERT INTO student_pictures (student_id, picture) VALUES (?, ?) ON DUPLICATE KEY UPDATE picture = ?';
      mysqlConnection.query(query, [student.studentNumber, picture, picture], (err) => {
        if (err) throw err;
        console.log('Picture uploaded successfully to MySQL');
      });
    } else {
      console.log('Deadline has passed, cannot upload picture');
    }
  }
};
